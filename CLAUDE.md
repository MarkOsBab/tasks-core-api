...
| Projects  | `/api/projects`  | `select?q=&clientId=`; filtros clientId/status; **`repositories[]`** (URLs/nombres de repos git — los usa el MCP para mapear working copy → proyecto); `GET /{id}/forecast` → **forecast data-driven sin LLM** (`src/domain/projects/project-forecast.service.ts` + helper compartido `src/domain/estimation/estimation-samples.ts`, el mismo ground truth del bloque `estimation` del asistente): horas estimadas restantes de tasks abiertas × ratio mediano trackeado/estimado (global y por label con ≥3 muestras), velocidad de los últimos 14 días y `projectedFinishDate` + confidence + notes; sin tasks abiertas o sin estimaciones → `{available:false, reason}`. El mismo bloque va en `get_project_summary` del MCP |
...
| Learnings | `/api/project-learnings` | **Solo lectura + retiro** de la memoria institucional que escribe el MCP (`add_learning`): `GET` listado paginado y `GET /{id}`, `DELETE` = soft delete (retirar un learning deja de alimentárselo a los agentes en `get_task`/`find_project`). Sin POST/PUT: los learnings nacen por MCP. Filtros `projectId`, `clientId` (vía `project.clientId`), `taskId`, `createdById`, `viaAgent=true\|false` + `search` sobre el body; orden default `createdAt desc`. El resource expone project/client (id, nombre, color), la task de origen, el autor y `viaAgent`. Lo consume el panel `/learnings` de la UI |
| Dashboard | `/api/dashboard` | counts (`totalClients`, `activeProjects`, `pendingProposals`, `openTasks`, `tasksByColumn`) |
| AI        | `/api/ai/task-drafts` | **Asistente IA (OpenAI, `src/lib/ai.ts` — cliente lazy estilo mailer; env `OPENAI_API_KEY` + `AI_MODEL`, sin key → 503).** `POST /task-drafts` `{input, projectId?}`: `ai-context.service` arma un snapshot de TODA la DB (columnas del board global, labels+uso, users con **rol + memberProjects + historial agregado de las últimas 200 tasks** — assignedTaskCount/topLabels/trackedHours/recentProjects — + carga abierta, projects activos, 40 tasks recientes con labels/asignados/checklist/horas trackeadas, propuestas) y OpenAI (structured outputs strict) devuelve borradores `{title, description HTML, priority, columnId, projectId, labelIds/labels, assigneeIds/assignees, checklist[], estimatedHours, reasoning}` saneados contra el snapshot (ids inventados se descartan). **`estimatedHours` se persiste como `Task.estimatedHours` en el apply y se estima a ritmo AI-assisted**: el prompt asume que las tarjetas de desarrollo las implementan agentes IA (Claude Code) supervisados — el grueso del tiempo es review/integración/verificación, no tipeo; tarjetas no-dev mantienen ritmo humano. **Regla de asignación: solo con evidencia** (membresía en el proyecto de la tarjeta > rol compatible > historial de labels/proyectos similares); la carga (`openTaskCount`) solo desempata entre calificados — nunca se asigna a alguien sin historial "porque está libre". Nada se persiste. Los borradores incluyen **`aiMetadata`** (acceptanceCriteria en texto plano, technicalNotes para el agente, targetRepos ⊆ repositories del proyecto, dependsOnIndexes intra-lote); el contexto lleva además **agregados por proyecto** (repositories, estimatedHours vs trackedHours, tasks open/done, topLabels, contributors) y un bloque **`estimation`** (hasta 30 tarjetas con estimado Y trackeado real cerrado — cuentan también las "en revisión", no solo terminales — + `medianTrackedToEstimateRatio`; `sampleTasks` lleva estimatedHours además de trackedHours): el prompt ancla cada estimación a los trackedHours de tarjetas comparables y escala por el ratio mediano (ratio <1 = estimaciones históricas infladas → bajar). `POST /task-drafts/apply` `{drafts[]}`: valida ids en batch y crea cada task vía `taskService.create` (WIP, posición, notificaciones) + checklist items; persiste `Task.aiMetadata` resolviendo dependsOnIndexes → `dependsOnTaskIds` reales en una segunda pasada. `maxDuration=300` en la ruta de generación. Además `GET /api/ai/activity` (`ai-activity.service.ts`, sin LLM): métricas del rastro `viaAgent` por rango de fechas (`?from=&to=`, default 30 días) — horas trackeadas IA vs humanas (`TimeEntry.viaAgent`, lo marca `start_tracking` del MCP), comentarios "Tasks IA", tarjetas creadas por agentes (`Task.viaAgent`, lo marca `create_task`) y learnings, más un feed `recent` mergeado de los 4 tipos; alimenta la página `/ai-activity` de la UI. |
| MCP       | `/api/mcp`       | **Servidor MCP (streamable HTTP, `mcp-handler` + SDK oficial; stateless, sin Redis, SSE off).** Auth: bearer = mismo JWT HS256 (emitir uno largo con `POST /api/auth/mcp-token`); `withMcpAuth` mete el userId en `authInfo.extra` para atribuir mutaciones. Tools (en `src/domain/mcp/`: `mcp-server.ts` registra, `mcp.service.ts` es la capa de datos — lecturas SIN LLM, razona el agente que llama): `find_project` (repo → proyecto vía `Project.repositories` con fallback heurístico por nombre; sin args lista todo con openTaskCount), `list_tasks` (default pendientes = columnas no terminales; cada ítem trae estimatedHours **y trackedHours** — señal para calibrar estimaciones nuevas contra duraciones reales), `get_task` (detalle implementation-ready: descripción HTML→texto vía `src/lib/html-text.ts`, checklist, comments, roles, estimatedHours vs trackedHours), `create_task` (crea una tarjeta vía `taskService.create` — WIP, posición al final, createdBy y notificaciones — con columna por nombre/id o default la primera del board, projectId validado, prioridad/estimatedHours/dueDate, `assignToMe`, checklist inicial con posiciones y spec `aiMetadata` opcional — acceptanceCriteria/technicalNotes/targetRepos/dependsOnTaskIds con ids validados contra tasks vivas; el resultado incluye los ids del checklist para `check_checklist_item`), `update_task` (edición parcial para grooming: solo los campos presentes se tocan — title/description/priority/estimatedHours/dueDate/`aiDelegable`; las claves del spec `aiMetadata` **mergean** sin pisar las no enviadas; `addChecklistItems` agrega ítems posicionados salteando títulos ya existentes; valida dueDate, dependencias vivas y auto-dependencia; los moves siguen en `move_task`), `move_task` (columna por nombre o id, cae al final, respeta WIP vía `taskService.move`), `comment_task` (vía `commentService.create` → notifica; marca `Comment.viaAgent` y el autor visible pasa a ser **"Tasks IA"** — `AGENT_COMMENT_AUTHOR` en `comment.constants.ts` — en el resource REST, las notificaciones y `get_task`; el `userId` real se conserva para permisos/auditoría), `check_checklist_item` (tilda/destilda un ítem del checklist vía `checklistItemService`; los ids salen del checklist de `get_task`; ítem inexistente → error 422 que lista los ítems reales de la task para autocorregirse), `start_tracking`/`stop_tracking` (timer del usuario del token vía `timeEntryService.start/stop`: un timer corriendo por usuario, start auto-cierra el anterior y lo reporta; **start marca `TimeEntry.viaAgent`** — igual que `create_task` marca `Task.viaAgent` — para separar horas IA vs humanas en `GET /api/ai/activity`; stop sin timer = resultado normal `stopped:false`, no error; stop devuelve sessionHours + taskTrackedHours para comparar con la estimación), `add_learning` (memoria institucional: registra UN hecho durable no-obvio del proyecto en `ProjectLearning` — projectId/taskId?/body≤2000/createdById/viaAgent, soft-delete —; dedupe exacto case-insensitive por proyecto → `duplicated:true`; los últimos 15 salen como `projectLearnings` en `get_task` y como `learnings` en los matches por working-copy de `find_project` para que cada corrida arranque sabiendo lo que las anteriores aprendieron; la skill /work-on-tasks manda leerlos antes de implementar y registrar al cerrar), `get_project_summary` (miembros+roles, horas est/track, tasks por columna/asignado, propuestas con montos, y bloque **`forecast`** data-driven — ver fila Projects), `list_columns`. `get_task` incluye **`aiMetadata`** (spec machine-readable de tarjetas generadas por IA: acceptanceCriteria, technicalNotes, targetRepos, dependsOnTaskIds). Los repos clientes traen `.mcp.json` sin credenciales apuntando a **prod** (`https://tasks-core-api.vercel.app/api/mcp` — misma DB; para probar cambios locales del MCP usar un override `--scope local` a localhost:3002): el cliente hace el flow **OAuth** (401 → WWW-Authenticate → discovery → consent en la UI). Ambos repos tienen la skill **`/work-on-tasks`** que orquesta find_project → list_tasks → get_task → implementar → comment + move. |
| Webhooks  | `/api/webhooks/github` | **PR ↔ tarjeta** (`src/domain/webhooks/github-webhook.service.ts`): recibe eventos `pull_request` de GitHub firmados con HMAC sha256 (`X-Hub-Signature-256` + env `GITHUB_WEBHOOK_SECRET`; firma inválida o secret ausente → 401; el raw body se lee ANTES de parsear). Detecta la tarjeta por rama `task-<id>-*` (patrón del runner IA) o mención `#<id>` en título/body. PR opened/reopened/ready_for_review → guarda `Task.prUrl` (migración `task_pr_url`; expuesto en el resource REST y en `get_task` del MCP), mueve la tarjeta a la **última columna no terminal** (revisión) y comenta el link `viaAgent` ("Tasks IA", autor = creador de la tarjeta o primer user); closed con `merged=true` → mueve a la columna terminal + comentario. Acciones ignoradas y errores de reglas de board (WIP) responden 200 con `handled:false` para que GitHub no reintente. |
...
## Runner autónomo (GitHub Actions)

`.github/workflows/ai-runner.yml` (+ `.github/ai-runner-prompt.md`, el brief del agente): cron
nocturno + `workflow_dispatch` que corre **Claude Code headless** con la suscripción (no API key).
Toma **una** tarjeta `aiDelegable` pendiente del proyecto de este repo (filtra dependencias sin
resolver y `targetRepos` ajenos), la mueve a "En progreso", trackea, implementa en una rama
`task-<id>-<slug>`, comenta y abre PR con `gh` — NO mueve la tarjeta a revisión: eso lo hace el
webhook de GitHub al abrirse el PR (y el merge la termina). Secrets requeridos en el repo:
`CLAUDE_CODE_OAUTH_TOKEN` (de `claude setup-token`) y `CORE_TASKS_MCP_TOKEN` (de
`POST /api/auth/mcp-token`); el MCP se registra en CI por header Authorization (sin OAuth
interactivo). El webhook del repo en GitHub debe apuntar a
`https://tasks-core-api.vercel.app/api/webhooks/github` (content type JSON, evento pull_request,
secret = `GITHUB_WEBHOOK_SECRET` del env de Vercel). El repo `core-tasks-ui` tiene el workflow
gemelo (cron desfasado 30 min para no pisarse el board).
