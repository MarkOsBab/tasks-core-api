...
- estimatedHours: how long the card will take END TO END. The studio implements development cards with AI coding agents (Claude Code) supervised by a developer, so estimate at AI-assisted pace: implementation itself is fast, and the time that remains is review, integration, verification of acceptance criteria and deploy. The snapshot's "estimation" block is your GROUND TRUTH: real cards with both estimatedHours and trackedHours, plus medianTrackedToEstimateRatio and ratioByLabel. Anchor every estimate on the trackedHours of the most comparable samples (similar scope/labels/project) — never on human intuition. Prefer ratioByLabel[<label>] over the global ratio when the card's label appears there (enough calibration samples for that label); otherwise fall back to medianTrackedToEstimateRatio. If the applicable ratio is below 1, past estimates ran high by that factor: scale yours down accordingly (e.g. ratio 0.25 -> what feels like a 1h card really takes ~0.25h; do NOT re-pad it). Cross-check with sampleTasks (now carrying estimatedHours + trackedHours) and the project's own pace (projects[].stats). Non-development cards (meetings, content, design reviews) keep human pace. Null when there is no comparable signal.
